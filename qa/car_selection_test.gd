...
func _run() -> void:
	...
	var molniya_speed_bar := float(selector.get("stat_bars")[1].value)
	...
	check(not str(selector.get("description_label").text).to_lower().contains("чёрн"), "Cadillac description does not force a body color")
	check(float(selector.get("stat_bars")[1].value) > molniya_speed_bar, "800 km/h Cadillac fills more of the speed bar than 650 km/h Molniya")
	check(is_equal_approx(float(selector.get("stat_bars")[1].max_value), 800.0), "speed bar uses the real maximum-speed scale")
