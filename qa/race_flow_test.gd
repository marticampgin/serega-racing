...
func _run() -> void:
	...
	check(str(game.get("refuel_label").text).begins_with("ЗАПРАВЛЯЕМ МАШИНУ"), "recording screen changes to a neutral animated fueling message")
	var dot_states: Array[String] = []
	for elapsed_value in [5.1, 5.6, 6.1]:
		game.set("refuel_request_elapsed", elapsed_value)
		game.call("_update_refuel_sequence", 0.0)
		dot_states.append(str(game.get("refuel_label").text))
	check(dot_states[0].ends_with("..") and dot_states[1].ends_with("...") and dot_states[2].ends_with(".") and dot_states[0] != dot_states[1] and dot_states[1] != dot_states[2], "fueling message loops smoothly through one, two, and three dots")
	check(bool(game._music_controller().get("ducked")), "fueling keeps the current music playing at reduced volume")
	...
	check(game.get("refuel_panel").visible and "GEMINI" not in game.get("refuel_label").text and "quota" not in game.get("refuel_label").text, "fueling failures keep backend details out of the game UI")
	check(not bool(game._music_controller().get("ducked")), "fueling restores normal music volume when processing ends")
	game.get("refuel_panel").hide()
	game.set("fuel", 20.0)
