...
func _run() -> void:
	...
	check(not game.get("mode_selector").get("powerups_toggle").visible, "obstacle mode hides its mandatory power-up toggle")
	game.get("mode_selector").call("_select_mode", "free_run")
	check(game.get("mode_selector").get("powerups_toggle").visible, "free run retains its optional power-up toggle")
