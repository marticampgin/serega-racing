...
func _run() -> void:
	...
	check(game.get("mode_selector").find_children("*", "CheckButton", true, false).is_empty(), "mode menu exposes no optional power-up toggle")
	game.get("mode_selector").call("_select_mode", "free_run")
	check(game.get("mode_selector").find_children("*", "CheckButton", true, false).is_empty(), "free run cannot enable power-ups")
