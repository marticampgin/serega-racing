...
func _run() -> void:
	...
	race.set("speed", 100.0)
	race.set("road_edge_contact_time", 0.5)
	race.call("handle_road_edge_contact", 0.0, 100.0, 0.1, false, false)
	check(float(race.get("speed")) < 90.0, "sustained wall sliding receives a heavy speed penalty")
	race.set("fuel", 12.0)
